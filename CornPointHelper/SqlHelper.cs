...
namespace CornPointHelper
{
    ...
    public sealed class SqlHelper
    {
        ...
        public static string ExecuteNonQueryWithOutPutParameterValue(SqlConnection connection, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            ...
            try
            {
                ...
                foreach (SqlParameter param in cmd.Parameters)
                {
                    ...
                    {
                        outParamValue = cmd.Parameters[param.ParameterName].Value.ToString();
                    }
                }
        ...
        public static void GetAppPath()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    DataRow[] drComInfo = dt.Select("CODE = '" + (int)Enums.AppSetting.ComputerInfo + "'");
                    string CInfo = Cryptography.Encrypt(ComputerInfo.Value(), "b0tin@74");
                    string[] ComInfo = (drComInfo[0]["VALUE"].ToString()).Split(',');
                    ...
                    DataRow[] drPath = dt.Select("CODE = '" + (int)Enums.AppSetting.AppPath + "'");
                    string[] Path = (drPath[0]["VALUE"].ToString()).Split(',');
                    Configuration.AppPath = Path[IndexOfInfo];
                }
                else
                {
