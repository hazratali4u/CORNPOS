...
namespace CORNDatabaseLayer.Classes
{
    public class spInsertSALE_INVOICE_DETAIL
    {
        ...
        public long ExecuteQueryForInvoiceUpdate()
        {
            try
            {
                IDbCommand cmd = ProviderFactory.GetCommand(EnumProviders.SQLClient);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "spUpdateSALE_INVOICE_DETAIL";
                cmd.Connection = m_connection;
                if (m_transaction != null)
                {
                    cmd.Transaction = m_transaction;
                }
                GetParameterCollectionUpdate(ref cmd);
                cmd.ExecuteNonQuery();
                m_SALE_INVOICE_DETAIL_ID = (long)((IDataParameter)(cmd.Parameters["@SALE_INVOICE_DETAIL_ID"])).Value;
                return m_SALE_INVOICE_DETAIL_ID;
            }
        ...
        public void GetParameterCollectionUpdate(ref IDbCommand cmd)
        {
            IDataParameterCollection pparams = cmd.Parameters;
            IDataParameter parameter;
