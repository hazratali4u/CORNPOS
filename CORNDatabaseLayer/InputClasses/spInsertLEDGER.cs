...
namespace CORNDatabaseLayer.Classes
{
    public class spInsertLEDGER
    {
        ...
        public bool ExecuteQueryDeleteLedger()
        {
            try
            {
                IDbCommand cmd = ProviderFactory.GetCommand(EnumProviders.SQLClient);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "spDeleteLEDGER";
                cmd.Connection = m_connection;
                if (m_transaction != null)
                {
                    cmd.Transaction = m_transaction;
                }
                GetParameterCollectionForDeleteLedger(ref cmd);
                cmd.ExecuteNonQuery();
                return true;
            }
            catch (Exception e)
        ...
        public DataTable ExecuteTableForVoucherByLedgerId()
        {
            try
            {
                IDbCommand command = ProviderFactory.GetCommand(EnumProviders.SQLClient);
                command.CommandType = CommandType.StoredProcedure;
                command.CommandText = "spSelectVoucherNoByLedgerID";
                command.Connection = m_connection;
                if (m_transaction != null)
                {
                    command.Transaction = m_transaction;
                }
                GetParameterCollectionForVocuherNo(ref command);
        ...
        public void GetParameterCollectionForVocuherNo(ref IDbCommand cmd)
        {
            IDataParameterCollection pparams = cmd.Parameters;
            IDataParameter parameter;
            ...
            parameter.ParameterName = "@Ledger_ID";
            parameter.DbType = ProviderFactory.GetDBType(EnumProviders.SQLClient, EnumDBTypes.BigInt);
            if (m_LEDGER_ID1 == Constants.LongNullValue)
            {
                parameter.Value = DBNull.Value;
            }
            else
            {
                parameter.Value = m_LEDGER_ID1;
            }
            pparams.Add(parameter);


        }
        public void GetParameterCollectionForDeleteLedger(ref IDbCommand cmd)
        {
            IDataParameterCollection pparams = cmd.Parameters;
            IDataParameter parameter;
