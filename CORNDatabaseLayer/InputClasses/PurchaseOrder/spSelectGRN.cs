	...
	public class spSelectGRN
    {
		...
		public DataTable ExecuteTable()
		{
			try
			{
				...
				command.CommandText = "spSelectGRN";
        ...
        public DataTable ExecuteTableForPurchaseOrderINGRN()
        {
            try
            {
                IDbCommand command = ProviderFactory.GetCommand(EnumProviders.SQLClient);
                command.CommandType = CommandType.StoredProcedure;
                command.CommandText = "spSelectGRNByPurchaseOrderID";
                command.Connection = m_connection;
                if (m_transaction != null)
                {
                    command.Transaction = m_transaction;
                }
                GetParameterCollectionFORPurchaseOrder(ref command);
                IDbDataAdapter da = ProviderFactory.GetAdapter(EnumProviders.SQLClient);
                da.SelectCommand = command;
                DataSet ds = new DataSet();
                da.Fill(ds);
        ...
        public void GetParameterCollectionFORPurchaseOrder(ref IDbCommand cmd)
        {
            try
            {
                IDataParameterCollection pparams = cmd.Parameters;
