...
public partial class Form_frmDiscountTemplate : System.Web.UI.Page
{
    ...
    private void LoadGridData()
    {
        ...
        dt = discountCtrl.GetDiscountTemplate(1,Constants.IntNullValue);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (Page.IsValid)
            {
                ...
                {
                    case "Save":
                        flag = discountCtrl.InsertDiscountTemplate(Convert.ToInt32(drpDistributor.SelectedItem.Value),txtDiscountName.Text, Convert.ToDecimal(DataControl.chkNull_Zero(txtDiscount.Text)), Convert.ToByte(ddlDiscountType.Value),Convert.ToInt32(Session["UserID"]));
                        ScriptManager.RegisterStartupScript(this, typeof(Page), "Alert", "alert('Record added successfully.');", true);
                        break;
                    case "Update":
                        discountCtrl.UpdateDiscountTemplate(Convert.ToInt32(hfBankDiscount.Value),txtDiscountName.Text, Convert.ToDecimal(DataControl.chkNull_Zero(txtDiscount.Text)), Convert.ToByte(ddlDiscountType.Value),Convert.ToInt32(Session["UserID"]));
        ...
        try
        {
            ...
            foreach (GridViewRow dr in GrdCustomer.Rows)
            {
                ...
                if (chRelized.Checked)
                {
                    ...
                    {
                        UserCntrl.ActiveInactive(false, Convert.ToInt32(dr.Cells[1].Text), Constants.IntNullValue, 13);

                        flag = true;
                    }
                    else
                    {
                        UserCntrl.ActiveInactive(true, Convert.ToInt32(dr.Cells[1].Text), Constants.IntNullValue, 13);
                        flag = true;
                    }
                }
                if (flag)
