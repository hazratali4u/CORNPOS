...
public partial class Forms_frmItemRecipeReplica : System.Web.UI.Page
{
    ...
    private void LoadSKUFinishedNew()
    {

        DataTable Dtsku_Price = SKUCtl.SelectSkuInfo(Constants.IntNullValue, Constants.IntNullValue, Constants.IntNullValue, 33, int.Parse(Session["CompanyId"].ToString()), null);
        clsWebFormUtil.FillDxComboBoxList(this.ddlSKUFinishedNew, Dtsku_Price, 0, 2, true);
        if (Dtsku_Price.Rows.Count > 0)
        {
            ddlSKUFinishedNew.SelectedIndex = 0;
        }
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        {
            DataTable dtSKU = (DataTable)Session["dtSKU"];
            DataTable deletedSKU = dtSKU.Clone();
            if (dtSKU.Rows.Count > 0)
            {
                int RecipeID = SKUCtl.InsertFinishedSKU(Convert.ToInt32(ddlSKUFinishedNew.SelectedItem.Value),
                    ...
                    chkIsReqProduciotn.Checked, dtSKU, deletedSKU);
                if (RecipeID > 0)
                {
                    LoadSKUFinishedNew();
                    CreateSKUDataTable();
