...
public partial class Forms_RptProductionIn : Page
{
    ...
    private void LoadItemName()
    {
        ...
        DataTable Dtsku_Price = SKUCtl.SelectSkuInfo(Constants.IntNullValue, Constants.IntNullValue, Constants.IntNullValue, 9, int.Parse(Session["CompanyId"].ToString()), null);
    ...
    private void ShowReport(int pReprotType)
    {
        ...
        {
            ds = _rptInventoryController.GetDateWiseProductionINReport(int.Parse(drpDistributor.SelectedItem.Value.ToString()),
                DateTime.Parse(txtStartDate.Text + " 00:00:00"), DateTime.Parse(txtEndDate.Text + " 23:59:59"),
                int.Parse(drpItemName.Value.ToString()));
            ...
            crpReport.SetParameterValue("ReportName", "Date Wise Production In Report (Qty)");
        ...
        {
            ds = _rptInventoryController.GetProductionINSummaryReport(int.Parse(drpDistributor.Value.ToString()),
                DateTime.Parse(txtStartDate.Text + " 00:00:00"), DateTime.Parse(txtEndDate.Text + " 23:59:59"),
                int.Parse(drpItemName.Value.ToString()));

            CrpProductionINSummary crpReport = new CrpProductionINSummary();
            crpReport.SetDataSource(ds);
            crpReport.Refresh();


            crpReport.SetParameterValue("DocumentType", "Issue To");
            crpReport.SetParameterValue("ReportName", "Prodcution In Summary");
