...
public partial class Forms_rptIncomeStatementBeirut : System.Web.UI.Page
{
    ...
    {
        DataSet ds = _rptSaleCtl.SelectRptGrossProfitBeirut(int.Parse(drpDistributor.SelectedItem.Value.ToString()), Convert.ToInt32(Session["UserID"]), DateTime.Parse(txtStartDate.Text), DateTime.Parse(txtEndDate.Text));
        DataTable dt = _mDocumentPrntControl.SelectReportTitle(int.Parse(drpDistributor.SelectedItem.Value.ToString()));

        var crpReport = new CrpGrossProfitReportBeirut();
        ...
        decimal Food = 0;
        decimal Beverage = 0;
        foreach (DataRow dr in ds.Tables["uspGetConsumptionAccountHeadWise"].Rows)
        {
            if(dr["AccountHead"].ToString() == "FOOD COST")
            {
                Food = Convert.ToDecimal(dr["Amount"]);
            }
            else if (dr["AccountHead"].ToString() == "BEVERAGE COST")
            {
                Beverage = Convert.ToDecimal(dr["Amount"]);
            }
        }
        crpReport.SetParameterValue("Food", Food);
        crpReport.SetParameterValue("Beverage", Beverage);
