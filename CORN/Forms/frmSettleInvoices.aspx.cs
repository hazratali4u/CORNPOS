...
public partial class Forms_frmSettleInvoices : System.Web.UI.Page
{
    /// <summary>
    /// Page_Load Function
    /// </summary>
    ...
    private void ShowReport(int pReportType)
    {
        ...
        DataSet ds = rptCustomerCtl.SelectInvoiceDetail(Convert.ToInt32(Session["UserID"]), invoiceId,int.Parse(drpDistributor.SelectedItem.Value.ToString()),DateTime.Parse(txtStartDate.Text + " 00:00:00"),DateTime.Parse(txtEndDate.Text + " 23:59:59"),6,Constants.IntNullValue,Constants.IntNullValue);
    ...
    protected void btnGetData_Click(object sender, EventArgs e)
    {
        ...
        var rptCustomerCtl = new RptCustomerController();
        DataTable dt2 = rptCustomerCtl.SelectInvoiceDetail2(Convert.ToInt32(Session["UserID"]),strInvoiceNo2,int.Parse(drpDistributor.SelectedItem.Value.ToString()), DateTime.Parse(txtStartDate.Text + " 00:00:00"), DateTime.Parse(txtEndDate.Text + " 23:59:59"),Constants.IntNullValue, 2,0,Constants.IntNullValue);
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        string invoiceId = (from GridViewRow dr in Grid_users.Rows let chbSelect = (CheckBox)(dr.Cells[0].FindControl("ChbCustomer")) where chbSelect.Checked select dr).Aggregate("0", (current, dr) => current + "," + dr.Cells[2].Text);
        if(Order.InActiveInvoice(invoiceId,Convert.ToInt32(drpDistributor.Value),Convert.ToDateTime(txtStartDate.Text + " 00:00:00"), Convert.ToInt32(Session["UserID"])))
        {
