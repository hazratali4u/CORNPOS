...
/// </summary>
public partial class Forms_RptSectionWiseCostSheet : System.Web.UI.Page
{
    /// <summary>
    /// Page_Load Function
    /// </summary>
    ...
    protected void ShowReport(int ReportType)
    {
        ...
        int TypeID = int.Parse(rblReportType.SelectedValue.ToString());
        ReportDocument CrpReport = new ReportDocument();

        if (TypeID == 3)
            CrpReport = new CrpDeptWiseCostSheet();
        else
            CrpReport = new CrpCategoryWiseCostSheet();

        DataSet ds = RptInventoryCtl.SelectRawIssuenceReport(
            int.Parse(drpDistributor.SelectedItem.Value.ToString()),
            DateTime.Parse(txtStartDate.Text + " 00:00:00"),
            DateTime.Parse(txtEndDate.Text + " 23:59:59"), "", TypeID, Constants.IntNullValue);

        CrpReport.SetDataSource(ds);
        CrpReport.Refresh();
        if (TypeID == 3)
            CrpReport.SetParameterValue("DocumentType", "Department Wise Cost Sheet");
        else
            CrpReport.SetParameterValue("DocumentType", "Category Wise Issuance Report");
