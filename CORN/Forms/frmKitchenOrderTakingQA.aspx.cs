...
public partial class Forms_frmKitchenOrderTakingQA : System.Web.UI.Page
{
    ...
    public static string SelectPendingBills()
    {
        ...
        DataTable distinctValues = view.ToTable(true, "SERVICE_TYPE", "ORDER_NO", "TABLE_NO2", "ORDER_DATE_TIME",
            "ORDER_TIME", "ORDER_BOOKER_NAME", "INVOICE_ID", "REMARKS", "HideKDSMarkCompleteBtn", "CUSTOMER_NAME",
            "ADDRESS", "CONTACT_NUMBER", "IsKDSOrderReady", "KDS_TIME", "covertable");
    ...
    [WebMethod]
    [ScriptMethod]
    public static void RecallKDSQA(string InvoiceId)
    {
        try
        {
            if (IsDayClosed())
            ...
            int distributerId = int.Parse(HttpContext.Current.Session["DISTRIBUTOR_ID"].ToString());
            OrderEntryController.RecallKDSQA(long.Parse(InvoiceId), distributerId);
        }
        catch (Exception ex)
        {
            throw ex;
    ...
    [WebMethod]
    [ScriptMethod]
    public static void UpdateItemsAsLoaded(string InvoiceId)
    {
        try
        {
            // int tableId = int.Parse(HttpContext.Current.Session["TableId"].ToString());
            ...
            OrderEntryController.Update_Items_AS_Loaded(
                long.Parse(InvoiceId), distributerId);
    ...
    [WebMethod]
    [ScriptMethod]
    public static void UpdateItemAsServed(string SALE_INVOICE_DETAIL_ID, string isReady)
    {
        try
        {
            if (IsDayClosed())
            ...
            OrderEntryController.Update_Item_AS_Served(
                long.Parse(SALE_INVOICE_DETAIL_ID), bool.Parse(isReady));
