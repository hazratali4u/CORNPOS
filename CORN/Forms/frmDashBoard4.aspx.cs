...
public partial class Forms_frmDashBoard4 : System.Web.UI.Page
{
    ...
    protected void dshViewer_DashboardLoading(object sender, DevExpress.DashboardWeb.DashboardLoadingEventArgs e)
    {
        try
        {
            ...
            {
                if (objDsMenu.Tables[0].Rows[0]["vbrFile4"] != null && !String.IsNullOrEmpty(Convert.ToString(objDsMenu.Tables[0].Rows[0]["vbrFile4"])))
                {
                    using (Stream ms = new MemoryStream((byte[])objDsMenu.Tables[0].Rows[0]["vbrFile4"]))
                    {
            ...
            else if (ddlDashboardType.SelectedItem.Value == "4")
            {
    ...
    private void GetDashboard()
    {
        try
        {
            ...
            objDsDashboard = CornPointHelper.DashboardClass.GetDashboardDataSet(objDsMenu.Tables[0].Rows[0]["strDashboardMainProcName"].ToString(), Convert.ToDateTime(Session["dtFromDashboard"]), Convert.ToDateTime(Session["dtToDashboard"]), Convert.ToDateTime(Session["dtFromDashboard2"]), Convert.ToDateTime(Session["dtToDashboard2"]),Convert.ToInt32(Session["UserID"]), Convert.ToInt32(ddlDashboardType.SelectedItem.Value));
