...
public partial class Forms_RptDayWisePurchaseItemReport : Page
{
    readonly DocumentPrintController _dPrint = new DocumentPrintController();
    readonly PurchaseController _rptPurchaseController = new PurchaseController();
    ...
    private void ShowReport(int pReprotType)
    {
        ...
        ds = _rptPurchaseController.GetDayWisePurchaseItems(int.Parse(drpDistributor.SelectedItem.Value.ToString()),
            DateTime.Parse(txtStartDate.Text + " 00:00:00"), DateTime.Parse(txtEndDate.Text + " 23:59:59"));

        var crpReport = new CrpDayWisePurchaseItem();
        crpReport.SetDataSource(ds);
        crpReport.Refresh();
        crpReport.SetParameterValue("Location", drpDistributor.SelectedItem.Text);
        crpReport.SetParameterValue("FROM_DATE", DateTime.Parse(txtStartDate.Text));
        crpReport.SetParameterValue("TO_DATE", DateTime.Parse(txtEndDate.Text));
        crpReport.SetParameterValue("ReportName", "Date Wise Item Purchase Report (Qty)");
