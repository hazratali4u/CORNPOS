...
public partial class Forms_rptDailyReconciliation : System.Web.UI.Page
{
    ...
    {
        DataSet ds = _rptSaleCtl.GetDailyReconciliation(int.Parse(drpDistributor.SelectedItem.Value.ToString()), Convert.ToInt32(Session["UserID"]), DateTime.Parse(txtStartDate.Text), DateTime.Parse(txtStartDate.Text));
        DataTable dt = _mDocumentPrntControl.SelectReportTitle(int.Parse(drpDistributor.SelectedItem.Value.ToString()));
        decimal TodaySale = 0;
        decimal PreviousDaySale = 0;
        foreach (DataRow dr in ds.Tables["uspGetDailyReconciliation"].Rows)
        {
            if(dr["DateType"].ToString()== "TodayDate")
            {
                TodaySale = Convert.ToDecimal(dr["Sales"]);
            }
            if (dr["DateType"].ToString() == "PrevDate")
            {
                PreviousDaySale = Convert.ToDecimal(dr["Sales"]);
            }
        }
        var crpReport = new CrpDailyReconciliation();
        crpReport.SetDataSource(ds);
        crpReport.Refresh();
        crpReport.SetParameterValue("Company_Name", dt.Rows[0]["COMPANY_NAME"].ToString());
        crpReport.SetParameterValue("From_Date", txtStartDate.Text);
        crpReport.SetParameterValue("Location", drpDistributor.SelectedItem.Text);
        crpReport.SetParameterValue("TodaySale", TodaySale);
        crpReport.SetParameterValue("PreviousDaySale", PreviousDaySale);
