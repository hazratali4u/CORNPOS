...
public partial class Forms_frmOrderPOSCode : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (ds.Tables[6].Rows.Count > 0)
            {
                ...
                hfPOSCodeAddToGrid.Value = ds.Tables[6].Rows[0]["POSCodeAddToGrid"].ToString();
    ...
    public static string HoldOrder(string orderedProducts, string orderBooker, string coverTable, string customerType, string CustomerName, string maxOrderNo, string printType, string tableName, string takeAwayCustomer, string bookerName, string tabId, string CustomerNo, string VoidBy, string manualOrderNo, string remarks, string Gst, string delChannel, string serviceCharges, string AdvanceAmount, string IsItemChanged)
    {
        ...
        try
        {
            ...
            if (dtValue != null && dtValue.Rows.Count > 0 && dtValue.Rows[0]["INVOICE_ID"].ToString().Trim() != "N/A")
            {
                ...
                OrderEntryController.HoldOrder(long.Parse(dtValue.Rows[0]["INVOICE_ID"].ToString()), Constants.IntNullValue, customerTypeId, tableId, 0, 0, decGST, 0, 0, true, userId, currentWorkDate, distributerId, 0, dtValue, int.Parse(orderBooker), coverTable, takeAwayCustomer, intVoidBy, manualOrderNo, remarks, Convert.ToInt32(serviceCharges), Convert.ToInt32(customerId), Convert.ToBoolean(HttpContext.Current.Session["InvoicePrinted"]), Convert.ToDecimal(HttpContext.Current.Session["GSTRate"]), Convert.ToDecimal(HttpContext.Current.Session["GSTRate"]), HttpContext.Current.Session["BillFormat"].ToString(), Convert.ToDecimal(AdvanceAmount), 0, 0, 0, 0, 0, HttpContext.Current.Session["LocationWiseRecipe"].ToString(), Convert.ToInt32(delChannel), CashImpact, CreditCard_Impact, Convert.ToBoolean(HttpContext.Current.Session["KDSImplemented"]), ItemChanged,1,dtInvoice,OldInvoiceJson, IsFinanceIntegrate, dtCOAConfig);
                DataRow drStock = dtStk.NewRow();
    ...
    public static string SelectSalesSummary(string fromDate, string toDate)
    {
        ...
        DataTable dt = rptSaleCtl.SelectSaleReport(int.Parse(HttpContext.Current.Session["DISTRIBUTOR_ID"].ToString()), int.Parse(HttpContext.Current.Session["UserId"].ToString()), Convert.ToDateTime(fromDate),Convert.ToDateTime(toDate), 1,HiddenReport);
        return GetJson(dt);
    }

    [WebMethod]
    [ScriptMethod]
    public static string SelectSalesDetail()
    {
        var rptSaleCtl = new RptSaleController();
        DataTable dt = rptSaleCtl.SelectSaleReport(int.Parse(HttpContext.Current.Session["DISTRIBUTOR_ID"].ToString()), int.Parse(HttpContext.Current.Session["UserId"].ToString()), DateTime.Parse(HttpContext.Current.Session["CurrentWorkDate"].ToString()), 2);
        return GetJson(dt);
    }

    [WebMethod]
    [ScriptMethod]
    public static string SelectItemSalesDetail()
    {
        var rptSaleCtl = new RptSaleController();
        DataTable dt = rptSaleCtl.SelectSaleReport(int.Parse(HttpContext.Current.Session["DISTRIBUTOR_ID"].ToString()), int.Parse(HttpContext.Current.Session["UserId"].ToString()), DateTime.Parse(HttpContext.Current.Session["CurrentWorkDate"].ToString()), 3);
        return GetJson(dt);
    }
