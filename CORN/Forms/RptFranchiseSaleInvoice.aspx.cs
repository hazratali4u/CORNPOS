...
public partial class Forms_RptFranchiseSaleInvoice : Page
{
    ...
    private void ShowReport(int pReprotType)
    {
        ...
        CrpFranchiseInvoice CrpReport = new CrpFranchiseInvoice();
        DataTable dt = mController.SelectReportTitle(int.Parse(drpDistributor.SelectedItem.Value.ToString()));
        DataSet ds = _franchiseController.SelectFranchiseReportByLocationDate(int.Parse(drpDistributor.SelectedItem.Value.ToString()),
        ...
        CrpReport.SetParameterValue("ReportName", "Franchise Sale Invoice");
